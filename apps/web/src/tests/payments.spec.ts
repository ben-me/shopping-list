...
describe("Payments on a List", () => {
  it("records a Payment offline: it appears immediately and is queued for Sync", async () => {
    ...
    const payment = await addPayment(db, list.id, "user-1", 1250, "2026-02-01T10:00:00.000Z");
  ...
  it("rejects a Payment without an amount or a date", async () => {
    ...
    await expect(addPayment(db, list.id, "user-1", 12.5, "2026-02-01")).rejects.toThrow(
      "Give the payment a positive amount",
    );
    await expect(addPayment(db, list.id, "user-1", 1250, "")).rejects.toThrow(
  ...
  it("edits a Payment's amount and date offline and queues the change for Sync", async () => {
    ...
    expect(updated).toMatchObject({ amountInCents: 990, paidAt: "2026-02-03T18:30:00.000Z" });
    const stored = (await db.getPayments(list.id))[0];
    expect(stored).toMatchObject({ amountInCents: 990, paidAt: "2026-02-03T18:30:00.000Z" });
    const pending = await db.pendingOutboxEntries();
  ...
  it("syncs a queued Payment to the server when the connection returns", async () => {
    ...
    const payment = await addPayment(db, list.id, "user-1", 1250, "2026-02-01T10:00:00.000Z");
    await updatePayment(db, payment, { amountInCents: 990 });
  ...
  it("keeps a Payment entry pending when the server is unreachable so Sync retries later", async () => {
    ...
    await addPayment(db, list.id, "user-1", 1250, "2026-02-01T10:00:00.000Z");
